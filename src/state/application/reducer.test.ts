...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainID.HmyMainnet, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainID.HmyMainnet]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainID.HmyMainnet, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainID.HmyMainnet]).toEqual(3)
    })
