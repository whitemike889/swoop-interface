...
describe('v1SwapArguments', () => {
  const USDC_WONE = new MockV1Pair('1000000', new TokenAmount(USDC, '1000000'))
  const DAI_WONE = new MockV1Pair('1000000', new TokenAmount(DAI, '1000000'))

  // just some random address
  const TEST_RECIPIENT_ADDRESS = USDC_WONE.liquidityToken.address
  ...
  it('exact eth to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WONE], HARMONY), CurrencyAmount.ether('100'))
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WONE], USDC, HARMONY), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60,
  ...
  it('exact token to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WONE, DAI_WONE], HARMONY), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60,
  ...
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WONE], HARMONY), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60,
  ...
  it('token to exact eth', () => {
    const trade = Trade.exactOut(new Route([USDC_WONE], USDC, HARMONY), CurrencyAmount.ether('100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60,
  ...
  it('token to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WONE, DAI_WONE], USDC), new TokenAmount(DAI, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60,
