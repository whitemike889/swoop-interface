...
export default function AddLiquidity({
  ...
  // if new, add pair to user liquidity pool pairs
  const [canAddPair, setCanAddPair] = useState<boolean>(false);
  ...
  // const [pairState, pair] = usePair(wrappedCurrency(currencyA, chainId) ?? undefined, wrappedCurrency(currencyB, chainId) ?? undefined)
    ...
    <>
              ...
              <AutoColumn gap={'md'}>
                ...
                >
                  <Text fontSize={20} fontWeight={500}>
                    {attemptingTxn && expertMode ? <Dots>Supplying</Dots> : error ?? 'Supply'}
                  </Text>
                </ButtonError>
              </AutoColumn>
            )}
      ...
      {pair && !noLiquidity && pairState !== PairState.INVALID ? (
        <AutoColumn style={{minWidth: '20rem', marginTop: '1rem'}}>
          <MinimalPositionCard showUnwrapped={oneCurrencyIsWONE} pair={pair} />
        </AutoColumn>
      ) : null}
    </>
  );
